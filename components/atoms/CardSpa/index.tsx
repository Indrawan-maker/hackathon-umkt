...
export default function CardSpa({ judulSpa, deskripsiSpa, gambarSpa }: {judulSpa: string; deskripsiSpa: string; gambarSpa: StaticImageData;}) {
    return (
        <>
        <div
        className="w-62 group rounded-2xl hover:scale-105 transition-transform duration-300 hover:border-2  hover:border-[#4B1A09] overflow-hidden">
                <article className="rounded-2xl relative">
                    <div className="w-62 h-42 rounded-2xl">
                    ...
                    <h2 className="block sm:hidden text-center mt-4 font-bold tracking-wider">{judulSpa}</h2>
                    <div className="absolute w-62 h-42 rounded-2xl hover:bg-linear-to-t from-[#1b1b1c] via-[#1b1b1c]/70 to-transparent shadow-lg bottom-0 left-0 p-4 text-white translate-y-6 group-hover:opacity-100 opacity-0 group-hover:translate-y-0 ease-in-out transition-all duration-300 cursor-pointer">
